...
public class PageVO {
	...
	public PageVO(int pageNum, int amount, int total) {
		...
		//공식: (int)Math.ceil( 페이지 번호 / 화면에 보여질 페이지네이션 개수 ) * 화면에 보여질 페이지네이션 개수
		...
		//3. 실제 끝 번호 realEnd
		//만약 게시글이 52라면? -> 실제 끝 번호 6
		//만약 게시글이 163이라면? -> 실제 끝 번호 17
		//공식: (int)Math.ceil( 전체 게시글 수 / amount 개수 )
		this.realEnd = (int)Math.ceil( this.total / (double)this.amount );
		
		//4. endPage의 결정
		//예시: 131개 게시물
		//1번 페이지 클릭 -> endPage = 10, realEnd = 14 (10)
		//11번 페이지 클릭 -> endPage = 20, realEnd = 14 (14)
